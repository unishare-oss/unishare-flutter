...
When a post is created with a PDF or DOCX attachment, the Flutter client calls a Cloudflare Worker (`POST /ai/summarize`) that extracts the document text, sends it to the Groq LLM API, and returns a bullet-point summary. The client then writes the result back to the `posts/{postId}` Firestore document via `updatePostSummary`. The Flutter app surfaces this summary in a collapsible `AiSummaryPanel` widget on the post detail screen. Once the summary is ready, an `AskAiSection` widget allows the viewer to ask free-form questions about the document; answers are generated by a second Worker endpoint (`POST /ai/chat`) and the conversation is held ephemerally in Riverpod state only. Text-only posts (no `mediaUrls`) never show either widget.
